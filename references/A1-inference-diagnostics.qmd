---
title: "A1. Inference and diagnostics"
format: html
execute: {echo: true, warning: false, message: false}
---

::: {.callout-note title="Companions"}
[Inference and diagnostic mathematics](../math-writeups/05-inference-diagnostics.qmd) · [Residual diagnostics for GLMs](../analysis-in-r/09-diagnostics-residual.qmd)
:::

```{r}
#| label: setup
#| include: false
source("_common.R")
library(lmerTest)
library(pbkrtest)
library(influence.ME)
library(DHARMa)
library(performance)
library(MASS)
library(dplyr)
library(ggplot2)
library(lme4)
library(nlme)
```

## 1. Aim

This appendix consolidates inferential and diagnostic operations by model class. Diagnostics target mean/link specification, residual covariance, random-effects adequacy, cluster influence, numerical convergence, and distributional dispersion; no single residual definition diagnoses all components.

## 2. Model specification

For an LMM, marginal residuals are $r_i^{(m)}=Y_i-X_i\widehat\beta$ and conditional residuals are $r_i^{(c)}=Y_i-X_i\widehat\beta-Z_i\widehat b_i$. For GLM/GLMM mean $\widehat\mu$, Pearson residuals are $(Y-\widehat\mu)/\sqrt{\widehat{\operatorname{Var}}(Y)}$; deviance residuals are signed square roots of unit-deviance contributions. Cluster influence compares $\widehat\beta$ with $\widehat\beta_{(-i)}$.

## 3. R packages/functions

`residuals()` exposes model-specific residuals; `nlme::Variogram()` assesses remaining lag dependence; `lme4::isSingular()` and optimizer output assess fitted random-effects geometry; `influence.ME` supplies cluster deletion; `DHARMa` supplies simulation-based GLMM diagnostics; `performance` offers convenient standardized checks.

## 4. Data

`Orthodont` supplies a Gaussian LMM example and `MASS::epil` supplies a repeated-count GLMM example.

```{r}
data("Orthodont", package = "nlme")
orth <- as.data.frame(Orthodont) |>
  # Sort by subject/time; Subject is cluster ID, age_c is centered time, visit is within-ID order.
  arrange(Subject, age) |>
  mutate(Subject = factor(Subject), age_c = age - 8, visit = ave(age, Subject, FUN = seq_along))

data("epil", package = "MASS")
epil <- MASS::epil |>
  # subject identifies repeated observations; period_c makes the first period time zero.
  mutate(subject = factor(subject), period_c = period - 1)
```

## 5. Fit

```{r}
# lmer(): distance is response; Sex*age_c is fixed mean; (age_c | Subject) is random intercept/slope.
lmm <- lme4::lmer(distance ~ Sex * age_c + (age_c | Subject), orth, REML = TRUE)
# gls(): corAR1(~ visit | Subject) specifies AR(1) residual correlation within subject.
gls_ar1 <- gls(
  distance ~ Sex * age_c, orth,
  correlation = corAR1(form = ~visit | Subject), method = "REML"
)
# glmer(): y is count response; family chooses Poisson/log; (1 | subject) is random intercept.
count_glmm <- lme4::glmer(
  y ~ trt * period_c + lbase + lage + (1 | subject),
  epil, family = poisson, nAGQ = 1
)
```

## 6. Extract & interpret

```{r}
# Convert the lme4 fit so anova(ddf = ...) can choose Satterthwaite or Kenward-Roger df.
lmm_lt <- lmerTest::as_lmerModLmerTest(lmm)
anova(lmm_lt, ddf = "Satterthwaite")
anova(lmm_lt, ddf = "Kenward-Roger")

lmm_reduced <- lme4::lmer(distance ~ Sex + age_c + (age_c | Subject), orth, REML = TRUE)
# KRmodcomp(full, reduced) tests fixed terms present only in the full model.
pbkrtest::KRmodcomp(lmm, lmm_reduced)

# parm = "theta_" requests variance parameters; profile uses profile-likelihood intervals.
confint(lmm, parm = "theta_", method = "profile")
```

Fixed-effect tests require the intended estimable function and denominator-df method. Variance-component CIs should respect the zero boundary. For nested fixed-effect LMM comparisons refit under ML unless using a KR comparison specifically formulated for REML fits with common random structure.

## 7. Diagnostics

### Gaussian marginal and conditional residuals

The normal Q--Q plots below are specific to Gaussian model diagnostics. Do not apply them to Pearson or deviance residuals from non-Gaussian GLMs or GLMMs.

```{r}
# fitted/resid are conditional on BLUPs; predict(re.form = NA) sets random effects to zero.
lmm_diag <- data.frame(
  fitted_conditional = fitted(lmm),
  residual_conditional = resid(lmm),
  residual_pearson = resid(lmm, type = "pearson"),
  fitted_marginal = predict(lmm, re.form = NA),
  residual_marginal = orth$distance - predict(lmm, re.form = NA)
)

# ggplot(data, aes(x, y)) maps fitted and Pearson residual columns to axes.
ggplot(lmm_diag, aes(fitted_conditional, residual_pearson)) +
  geom_point() + geom_hline(yintercept = 0, linetype = 2)
qqnorm(lmm_diag$residual_conditional); qqline(lmm_diag$residual_conditional)

re <- ranef(lmm)$Subject
qqnorm(re[["(Intercept)"]]); qqline(re[["(Intercept)"]])
qqnorm(re[["age_c"]]); qqline(re[["age_c"]])
```

### Serial dependence and influence

```{r}
# Variogram(model, ~ time | ID) summarizes normalized residual dependence by lag.
vg <- Variogram(gls_ar1, form = ~age | Subject, resType = "normalized")
plot(vg, smooth = TRUE)

# influence(model, group) deletes one independent Subject at a time.
infl <- influence.ME::influence(lmm, group = "Subject")
cd <- cooks.distance(infl)
head(sort(cd, decreasing = TRUE))
dfbetas(infl)[1:5, , drop = FALSE]
```

### GLMM residuals, dispersion, and convergence

```{r}
# Collect fitted values plus Pearson/deviance residuals for observation-level checks.
glmm_diag <- data.frame(
  fitted = fitted(count_glmm),
  pearson = residuals(count_glmm, type = "pearson"),
  deviance = residuals(count_glmm, type = "deviance")
)
ggplot(glmm_diag, aes(fitted, pearson)) + geom_point() + geom_hline(yintercept = 0, lty = 2)

# Pearson sum of squares divided by approximate residual df screens for overdispersion.
sum(glmm_diag$pearson^2) / (nobs(count_glmm) - length(fixef(count_glmm)))
isSingular(count_glmm)
count_glmm@optinfo$conv$lme4$messages

# simulateResiduals(model, n) generates simulation-based GLMM residuals; tests target dispersion/zeros.
dh <- DHARMa::simulateResiduals(count_glmm, n = 250, plot = FALSE)
plot(dh)
DHARMa::testDispersion(dh)
DHARMa::testZeroInflation(dh)

# check_model(model) uses see as an optional plotting backend; retain a numeric fallback when absent.
if (requireNamespace("see", quietly = TRUE)) {
  performance::check_model(count_glmm)
} else {
  performance::check_overdispersion(count_glmm)
}
```

## 8. Caveats

- Marginal residuals retain random-effect variation; conditional residuals depend on estimated BLUPs and can mask random-effect misspecification.
- QQ plots of BLUPs do not directly test the latent random-effects distribution because BLUPs are heteroscedastic, correlated, and shrunken.
- Pearson/deviance residuals are discrete and nonnormal in GLMMs; simulation-based residuals are usually more interpretable.
- Cook's distance and DFBETAs should be deleted by independent cluster, not by row, for longitudinal data.
- A flat residual variogram is evidence against remaining second-order lag structure only after the mean and random effects are adequate.
- Singularity is a boundary estimate, not an optimizer failure; convergence warnings require gradient/Hessian and rescaling checks before changing optimizers.
- SAS `PROC MIXED` influence measures and R package measures can use different scaling; compare rankings and deleted estimates, not labels alone.
- **Source-backed:** residual taxonomy, variograms, and influence follow BIOS 767/STAT 571. **Standard-practice addition:** `DHARMa`, `performance`, and `influence.ME` provide native R diagnostics.

## 9. Source references

- `BIOS-767-Part-012.pdf`, pp. 5--14 and 26--50 (marginal/conditional residuals, variograms, influence).
- UW STAT571 (serial correlation, residuals, and random-effect checks).
- `BIOS_767_Weideman_Typed_Notes_with_Examples.pdf`, pp. 86--92 (cumulative residuals and cluster diagnostics).
- BIOS767 Homework 4 (observation/case deletion influence and KR inference).
- BIOS767 Homework 5 (residual, QQ, and influence code).
